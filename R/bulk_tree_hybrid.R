...
#' Hybrid MCMC sampler for tree matrix and clonal prevalence.
#'
#' @export
bulk_tree_hybrid <- function(A, D, B_list, n_iter=1000, trans_mat=NULL,
  ...
  tree_prob <- rep(0, S)
  var_prob_mat <- matrix(0, nrow = N, ncol = K)
  for (i in seq(1, n_iter)) {
    # sample tree with Gibbs sampler: multinomial
    for (ii in seq_len(length(B_list))) {
      tree_prob[ii] <- log_post(A, D, Psi_cur, B_list[[ii]], Cluster_var)
    }
    tree_prob <- exp(tree_prob - max(tree_prob))
    tree_prob <- tree_prob / sum(tree_prob)
    tree_idx <- sample(S, 1, replace = FALSE, prob = tree_prob)
    tree_all[i] <- tree_idx

    # sample variants clustering with Gibbs sampler: multinomial
    if (!collaps_C) {
      var_prob_mat <- matrix(0, nrow = N, ncol = K)
      Cluster_var = NULL
    }

    # sample clonal prevalence with Metropolis Hasting sampler: logistic-norm
    for (m in seq_len(M)) {
      Phi_new <- Phi_cur
      Psi_new <- Psi_cur
      Phi_new[,m] <- mvtnorm::rmvnorm(n = 1, Phi_cur[,m], cov)
      Psi_new[,m] <- exp(Phi_new[,m]) / (sum(exp(Phi_new[,m])) + 1)
      Q_cur <- logistic_normal_pdf(Psi_cur[,m], Phi_new[,m], cov, log = TRUE)
      Q_new <- logistic_normal_pdf(Psi_new[,m], Phi_cur[,m], cov, log = TRUE)

      P_new <- log_post(A, D, Psi_new, B_list[[tree_idx]], Cluster_var)
      if (P_new > -0.05) {print(c(i, P_new))}
    ...
    samp_info[i,] <- c(i, accept_all[i], tree_idx, alpha, P_new, P_cur, Q_cur, Q_new)
  }
  #print(samp_info)
